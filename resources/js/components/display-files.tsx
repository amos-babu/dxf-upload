        ...
        <>
            {items.map((file, idx) => (
                <Link key={`${file.id}-${idx}`} href={route('files.show', { file: file.id })}>
                    <figure className="max-w-lg rounded-lg bg-muted transition-transform duration-300 hover:scale-105 hover:rounded-2xl hover:shadow-lg">
                        <img className="h-auto max-w-full rounded-t-md" src={file.image} alt={file.name} />
